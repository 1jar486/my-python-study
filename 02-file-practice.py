...
def main():
    init_db()
    current_path = os.path.dirname(os.path.abspath(__file__)) # 获取当前路径，让脚本自动识别自己在哪个文件路径
    ...
    for file in os.listdir(current_path):
        ...
        if os.path.exists(target_file_path): # 如果新文件路径已经存在了，说明这个文件夹里已经有一个同名的文件了      
            new_name = f"{folder_name}_{count}{ext}" # 新文件名，格式是：文件夹名字_数字.后缀名
            target_file_path = os.path.join(target_folder, new_name) # 新文件路径，放在以文件后缀名命名的文件夹里
            count += 1
...
>>>>>>> 7f4e3da281b5dafdd0a9405f46b8a55bada7df4e
